...
$(document).ready(function () {
	...
	sendMessageInputListener(); //메시지 엔터 이벤트
	sendMessageBtnListener(); //메세지이후 버튼 클릭이벤트.
...
function setupUsernameInputEventListener() {
	...
	.then((data)=> {
		...
		client.joinChannel({channelId: DEMO_CHANNEL_ID})
		.then(
			(data)=>{
			console.log('채널 들어가기');
			client.getMessages({channelId: DEMO_CHANNEL_ID})
			.then(data =>{
				populateChatWindowWithMessages(data.messages);
			})
			.catch(err=>{
		...
		.catch(err=> {
			if(err.code==='2003'){
				client.createChannel({  // if channel not found, create it
					channelId: DEMO_CHANNEL_ID,
					name: DEMO_CHANNEL_ID,
					type: 'super_public',
					members: []
			...
			}else if(err.code==='2008'){}// if user already had joined channel before, don't worry about error
			else{
				console.log(JSON.stringify(err));
			}
		})
